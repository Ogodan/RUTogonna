...
def filter_data(data,select_designations):
    if 'All' in select_designations:
        return data
    else:
        filter_condition = data['Designation'].isin(select_designations)
...
def main():
    ...
    if dept:
        ...
        select_designations = [option for option in options if st.sidebar.checkbox(option)]

    if st.sidebar.button('Submit'):
        final_data = filter_data(data,select_designations)
